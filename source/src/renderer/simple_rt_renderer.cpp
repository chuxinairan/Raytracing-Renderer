...
glm::vec3 SimpleRTRenderer::renderPixel(const glm::ivec2 &pixelCoord) {
  ...
  while (max_bounce_count--) {
    ...
    if (hit_info.has_value()) {
      ...
      if (hit_info->material->is_specular) {
        ...
        light_direction =
            glm::vec3(-view_direction.x, view_direction.y, -view_direction.z);
      } else {
        do {
          light_direction =
              glm::vec3(rng.uniform(), rng.uniform(), rng.uniform());
          light_direction = 2.0f * light_direction - 1.0f;
